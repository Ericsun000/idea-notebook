...
- **AI 增强** — 内置 GLM-4.7 开箱即用，支持接入大模型（DeepSeek/GLM/Qwen/Kimi/本地 Llama），一键生成每日/每周/每月总结、标签校准、深度讨论、蓝方辩论、项目回顾
- **后台运行** — 讨论/辩论提交后后台运行，不阻塞界面，完成后自动刷新想法卡片
- **周/月总结** — 支持生成本周、本月 AI 周期回顾，追踪思维趋势演变
...
3. 支持多个功能：
   - **总结** — 生成每日/每周/每月智能总结，支持三种时间范围切换
   - **校准** — 修正标签和分类
   - **讨论** — 对每条想法生成深度评论（后台运行，不阻塞操作，支持多模型交叉评论）
   - **辩论** — 蓝方辩手视角，只提出反面意见和质疑（后台运行，完成后自动展现）

...
- **AI 增强** — 内置 GLM-4.7 开箱即用，支持接入大模型（DeepSeek/GLM/Qwen/Kimi/本地 Llama），一键生成每日总结、标签校准、深度讨论、蓝方辩论、项目回顾
- **多模型并存** — 同时连接多个 LLM（如 DeepSeek Flash + DeepSeek Pro），每个功能独立选择模型
- **项目管理** — 创建项目，输入时关联想法到项目，项目级别 AI 总结（独立模型选择，自动保存到项目详情页），讨论时自动附项目上下文
- **每日笔记** — 汇总当天想法，生成结构化笔记
- **时间线** — 按日期浏览所有历史想法
- **回收站** — 软删除 + 3 天自动清除，支持恢复
- **完成标记** — 勾选完成，过滤进行中/已完成
- **项目选择** — 项目下拉菜单自动检测屏幕空间，空间不足时向上展开
- **深色模式** — 跟随系统自动切换
